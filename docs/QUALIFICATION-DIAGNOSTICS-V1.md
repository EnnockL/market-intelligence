...
`jackpot-qualification-policy-v1` evaluates every immutable Jackpot candidate revision independently. It does not change candidate state and does not recommend threshold changes.
...
Low relationship or token-risk coverage is a data gap (`UNKNOWN`), not evidence that the asset itself failed. Holder growth is `NOT_APPLICABLE` while no holder provider is available. Thresholds live only in `QUALIFICATION_POLICY_V1`; workers and UI must not duplicate them.
