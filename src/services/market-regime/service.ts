...
export class MarketRegimeService {
  ...
  async run(cutoff = new Date().toISOString()) {
    ...
    for (const assetClass of ["stock", "crypto"] as RegimeAssetClass[]) {
      let expectedAssets = 0, afterId: string | null = null;
      const observations: RegimeObservationPair[] = [];
      // Keyset pagination avoids PostgREST's response cap and bounds each database query.
      for (;;) {
        let query = this.db.from("assets").select("id").eq("kind", assetClass).eq("is_active", true).order("id").limit(500);
        if (afterId) query = query.gt("id", afterId);
        const { data: assets, error } = await query;
        if (error) throw error;
        if (!assets?.length) break;
        expectedAssets += assets.length;
        observations.push(...await this.observations(assetClass, assets.map((asset: { id: string }) => asset.id), cutoff));
        afterId = assets[assets.length - 1].id;
        if (assets.length < 500) break;
      }
      const result = classifyMarketRegime({ assetClass, evaluatedAt: cutoff, expectedAssets, observations });
  ...
  private async observations(assetClass: RegimeAssetClass, assetIds: string[], cutoff: string): Promise<RegimeObservationPair[]> {
    ...
    const { data, error } = await this.db.rpc("market_regime_observation_pairs_for_assets", { p_asset_ids: assetIds, p_asset_class: assetClass, p_cutoff: cutoff, p_lookback_seconds: Math.floor(lookback / 1000) });
    if (error) throw error;
