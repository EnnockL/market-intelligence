...
export class SimulationService {
  ...
  async run(request:CreateSimulationRequest) {
    ...
    const {data:run,error:runError}=await this.db.from("simulation_runs").insert({user_id:null,run_scope:"SYSTEM_RESEARCH",strategy:request.policy,status:"running",initial_capital_sek:request.initialCapitalSek,starts_at:request.startsAt,ends_at:request.endsAt,information_cutoff_at:request.startsAt,assumptions:policy,scoring_version:"jackpot-qualification-policy-v1",engine_version:SIMULATION_ENGINE_VERSION,qualification_policy_version:"jackpot-qualification-policy-v1",paper_policy_version:PAPER_ENGINE_VERSION,fee_model_version:FEE_MODEL_VERSION,slippage_model_version:SLIPPAGE_MODEL_VERSION,fx_policy_version:"historical-fx-policy-v1",information_cutoff_policy:INFORMATION_CUTOFF_POLICY,candidate_dataset_hash:candidateDatasetHash,market_dataset_hash:marketDatasetHash,input_hash:inputHash,currency:"SEK"}).select("id").single(); if(runError)throw runError;
