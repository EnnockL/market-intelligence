...
export interface ExecutionProvider{readonly name:string;readonly mode:ExecutionMode;health():Promise<{status:"HEALTHY"|"DEGRADED"|"FAILED";credentialsValid:boolean;tradePermission:boolean;withdrawPermission:boolean|null}>;getAccountState():Promise<ProviderAccountState>;getFillsPage?(request:ProviderFillPageRequest):Promise<ProviderFillPage>;placeOrder(order:ExecutionOrderRequest):Promise<ProviderOrder>;getOrder(instrumentId:string,clientOrderId:string,providerOrderId:string|null):Promise<ProviderOrder|null>;cancelOrder(instrumentId:string,clientOrderId:string,providerOrderId:string|null):Promise<ProviderOrder>}
