...
import { readOkxAccountEvidence } from "./okx-account-evidence";
...
interface OkxConfig{perm:string;acctLv:string;uid?:string}
...
export class OkxDemoExecutionProvider implements ExecutionProvider{
  ...
  async health(){try{const config=(await this.request<OkxConfig>("GET","/api/v5/account/config"))[0];if(!config)return{status:"FAILED"as const,credentialsValid:false,tradePermission:false,withdrawPermission:null};const permissions=config.perm.split(",").map(x=>x.trim().toLowerCase()),tradePermission=permissions.includes("trade"),withdrawPermission=permissions.includes("withdraw"),spotMode=config.acctLv==="1";return{status:tradePermission&&!withdrawPermission&&spotMode?"HEALTHY"as const:"DEGRADED"as const,credentialsValid:true,tradePermission,withdrawPermission,...(config.uid?{externalAccountId:config.uid}:{})}}catch{return{status:"FAILED"as const,credentialsValid:false,tradePermission:false,withdrawPermission:null}}}
  async getAccountEvidence(instrumentId:string,windowStart:string){return readOkxAccountEvidence(path=>this.request<unknown>("GET",path,undefined,10_000),instrumentId,windowStart)}
