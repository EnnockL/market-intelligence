...
import { DEFAULT_SCHEDULER_RUNTIME_MS, schedulerBatchLimit, schedulerError, schedulerHasTime } from "@/domain/forecast-scheduler";
...
export interface SchedulerOptions { maxRuntimeMs?: number }

export class ForecastSchedulerService {
  private readonly maxRuntimeMs: number;

  constructor(private db: SupabaseClient, private workerId = `scheduler-${process.pid}`, private news?: SchedulerNewsConfig, private ingestion?: SchedulerIngestionConfig, options: SchedulerOptions = {}) {
    this.maxRuntimeMs = Math.max(5_000, options.maxRuntimeMs ?? DEFAULT_SCHEDULER_RUNTIME_MS);
  }
  ...
  async runOnce(now = new Date().toISOString()) {
    const { data, error } = await this.db.rpc("claim_forecast_scheduled_job", { p_worker_id: this.workerId, p_now: now });
    if (error) throw error;
    const run = data?.[0];
    if (!run) return { claimed: false as const };
    const { data: job, error: jobError } = await this.db.from("scheduled_jobs").select("*").eq("id", run.job_id).single();
    ...
    const heartbeat = async () => {
      ...
      try { await this.db.rpc("heartbeat_forecast_scheduled_job", { p_run_id: run.id, p_worker_id: this.workerId, p_now: new Date().toISOString() }); }
      finally { heartbeatRunning = false; }
    };
    await heartbeat();
    const heartbeatMs = Math.max(5_000, Math.min(30_000, Number(job.lease_seconds ?? 180) * 333));
    const timer = setInterval(() => { void heartbeat(); }, heartbeatMs);
    timer.unref?.();
    try {
      const result = await this.execute(job, now);
      const records = Number(result.recordsProcessed ?? result.processed ?? result.evaluated ?? result.completed ?? result.created ?? result.revisions ?? result.inserted ?? 0);
      const finished = await this.db.rpc("finish_forecast_scheduled_job", { p_run_id: run.id, p_worker_id: this.workerId, p_success: true, p_records: records, p_metrics: result, p_error: null, p_now: new Date().toISOString() });
      if (finished.error) throw finished.error;
      return { claimed: true as const, jobKey: job.job_key, success: true, records, result };
    } catch (cause) {
      const message = schedulerError(cause);
      await this.db.rpc("finish_forecast_scheduled_job", { p_run_id: run.id, p_worker_id: this.workerId, p_success: false, p_records: 0, p_metrics: {}, p_error: message, p_now: new Date().toISOString() });
      return { claimed: true as const, jobKey: job.job_key, success: false, error: message };
    } finally {
      clearInterval(timer);
    }
  }
  ...
  private async execute(job: any, now: string): Promise<any> {
    ...
    if (job.job_type === "CRYPTO_MARKET") { if (!this.ingestion) throw new Error("INGESTION_PROVIDERS_NOT_CONFIGURED"); return runCryptoMarketIngestion(this.ingestion.cryptoProvider, repo); }
    ...
    if (job.job_type === "FORECAST_PERFORMANCE") return new ForecastPerformanceService(this.db).run(now);
    if (job.job_type === "SPECIALIST_AGENTS") return new SpecialistAgentService(this.db).run(100, now);
    if (job.job_type === "CATALYST_CLASSIFICATION") return new CatalystClassificationService(this.db).run(now);
    if (job.job_type === "CONSENSUS") return new ConsensusService(this.db).run(now);
