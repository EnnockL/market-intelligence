...
export class StrategyIntelligenceService {
  ...
  async researchEvaluation(evaluationRunId: string, options: { cutoffAt?: string } = {}) {
    ...
    const windowStart = trades.at(0)?.enteredAt ?? cutoff, windowEnd = trades.map((trade: StrategyTrade) => trade.exitedAt).sort().at(-1) ?? cutoff;
