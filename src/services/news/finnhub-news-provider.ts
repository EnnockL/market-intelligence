...
export class FinnhubNewsProvider implements NewsProvider {
  ...
  constructor(private apiKey:string,private fetcher:typeof fetch=fetch,private timeoutMs=12_000){}
  async fetchNews(request:NewsRequest):Promise<NewsProviderResult>{
    ...
    for(const symbol of [...new Set(request.symbols)].sort()){
      ...
      try{response=await this.fetcher(`https://finnhub.io/api/v1/company-news?symbol=${encodeURIComponent(symbol)}&from=${request.from.slice(0,10)}&to=${request.to.slice(0,10)}&token=${encodeURIComponent(this.apiKey)}`,{signal:AbortSignal.timeout(this.timeoutMs)})}catch(error){throw new NewsProviderError(`Finnhub news network failure: ${error instanceof Error?error.message:"unknown"}`,this.name,"unavailable",true)}
      ...
      const receivedAt=new Date().toISOString();for(const article of parsed.data.slice(0,request.limit)){
        const occurredAt=new Date(article.datetime*1000).toISOString(),providerArticleId=String(article.id),sourceClassification=classifyNewsSource(article.url);
        observations.push({observationKey:newsObservationKey({provider:this.name,providerArticleId,symbol}),provider:this.name,providerArticleId,symbol,headline:article.headline,summary:article.summary?.trim()||null,url:article.url,sourceName:article.source,sourceClassification,occurredAt,observedAt:occurredAt,availableAt:receivedAt,dataQuality:sourceClassification==="UNKNOWN"?50:80,rawPayload:article});
      }
    }
