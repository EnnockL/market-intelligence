...
export default async function WalletPage({ params }: { params: Promise<{ address: string }> }) {
  ...
  return <main className="wallet-page">
    ...
    <section className="wallet-kpis"><Kpi label="Discovery score" value={data.discovery ? `${data.discovery.score}/75` : "—"} /><Kpi label="Verified trades" value={String(metric?.verifiedTrades ?? 0)} /><Kpi label="Win rate" value={percent(metric?.winRate, true)} /><Kpi label="Realized P/L" value={money(metric?.realizedPnlUsd)} /><Kpi label="Max drawdown" value={percent(metric?.maxDrawdown, true)} /><Kpi label="Rug exposure" value={percent(metric?.rugExposureRate, true)} /><Kpi label="Risk coverage" value={metric ? `${metric.riskDataQuality}%` : "—"} /><Kpi label="Data quality" value={metric ? `${metric.dataQuality}%` : `${data.discovery?.dataQuality ?? 0}%`} /></section>
