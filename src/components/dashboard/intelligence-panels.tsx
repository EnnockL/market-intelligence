...
export function StockRadarPanel({ stocks }: { stocks: StockRadarItem[] }) {
  return <section className="radar-panel" id="stock-radar"><PanelTitle kicker="EQUITIES · FINNHUB" title="Stock Radar" /><div className="compact-list">{stocks.length ? stocks.map((item) => <div key={item.symbol}><div className="compact-symbol"><strong>{item.symbol}</strong><span>{item.catalyst}</span></div><span className={`direction direction--${item.signal}`}>{item.signal}</span><strong className="compact-score">{item.score ?? "—"}</strong><span className={item.change === null ? "neutral" : item.change >= 0 ? "positive" : "negative"}>{item.change === null ? "UNKNOWN" : `${item.change >= 0 ? "+" : ""}${item.change}%`}</span></div>) : <div className="dashboard-empty"><strong>No live stock snapshots</strong><span>Finnhub has not persisted any quotes yet.</span></div>}</div></section>;
}

export function RecentSignalsPanel({ signals }: { signals: RecentSignal[] }) {
  return <section className="radar-panel" id="signals"><PanelTitle kicker="SUPABASE · SIGNALS" title="Recent Signals" /><div className="signal-list">{signals.length ? signals.map((item) => <div key={item.id}><time>{item.occurredAt}</time><strong>{item.symbol}</strong><span>{item.label}</span><b className={item.scoreImpact === null ? "neutral" : item.scoreImpact >= 0 ? "positive" : "negative"}>{item.scoreImpact === null ? "—" : `${item.scoreImpact >= 0 ? "+" : ""}${item.scoreImpact}`}</b></div>) : <div><span>No persisted signals yet</span></div>}</div></section>;
}
