...
export interface WalletDetailData {
  ...
  progress: { key: string; value: number | boolean; required: number | boolean; remaining: number; passed: boolean }[];
...
export async function getWalletDetailData(address: string): Promise<WalletDetailData> {
  const fallback = (message: string): WalletDetailData => ({ address, found: false, tracked: false, label: null, discovery: null, metric: null, cycles: [], transactions: [], scoreV3: null, verification: null, progress: [], mode: "degraded", message });
  ...
  try {
    ...
    let metric: WalletDetailMetric | null = null; let cycles: WalletDetailCycle[] = []; let transactions: WalletDetailTransaction[] = []; let scoreV3: WalletDetailData["scoreV3"] = null; let verification: WalletDetailData["verification"] = null; let progress: WalletDetailData["progress"] = [];
    if (wallet) {
      const [{ data: metricRow, error: metricError }, { data: cycleRows, error: cycleError }, { data: transactionRows, error: transactionError }, { data: scoreRow, error: scoreError }, { data: verificationRow, error: verificationError }, { data: progressRow, error: progressError }] = await Promise.all([
        ...
        db.from("wallet_verification_progress").select("progress").eq("wallet_id", wallet.id).order("calculated_at", { ascending: false }).limit(1).maybeSingle(),
      ]);
      if (metricError) throw metricError; if (cycleError) throw cycleError; if (transactionError) throw transactionError; if (scoreError) throw scoreError; if (verificationError) throw verificationError; if (progressError) throw progressError;
      ...
      progress = verificationProgress(progressRow?.progress);
    ...
    return { address, found: true, tracked: Boolean(wallet?.is_tracked), label: wallet?.label ?? null,
      ...
      metric, cycles, transactions, scoreV3, verification, progress, mode: "live", message: metric?.verifiedTrades ? `${metric.verifiedTrades} verified trades` : "Evidence incomplete; verification blockers are shown below" };
...
function verificationProgress(value: unknown): WalletDetailData["progress"] { return Array.isArray(value) ? value.filter((item): item is WalletDetailData["progress"][number] => Boolean(item) && typeof item === "object" && "key" in item && "passed" in item) : []; }
