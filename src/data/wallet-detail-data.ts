...
export async function getWalletDetailData(address: string): Promise<WalletDetailData> {
  const fallback = (message: string): WalletDetailData => ({ address, found: false, tracked: false, label: null, discovery: null, metric: null, cycles: [], transactions: [], mode: "degraded", message });
  ...
  try {
    ...
    return { address, found: true, tracked: Boolean(wallet?.is_tracked), label: wallet?.label ?? null,
      ...
      metric, cycles, transactions, mode: "live", message: metric?.verifiedTrades ? `${metric.verifiedTrades} verified trades` : "Historical pricing incomplete; no verified performance yet" };
