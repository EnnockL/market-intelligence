...
export async function getQualificationData() {
  try {
    ...
    return { mode: "live" as const, policyVersion: snapshot?.policy_version ?? evaluations?.[0]?.policy_version ?? "jackpot-qualification-policy-v1", funnel: snapshot?.funnel ?? {}, blockerFrequency: snapshot?.blocker_frequency ?? {}, unknownFrequency: snapshot?.unknown_frequency ?? {}, cutoff: snapshot?.information_cutoff_at ?? null,
  ...
  } catch { return { mode: "degraded" as const, policyVersion: "jackpot-qualification-policy-v1", funnel: {}, blockerFrequency: {}, unknownFrequency: {}, cutoff: null, candidates: [] }; }
}
