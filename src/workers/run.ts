...
import { runStrategyResearchCycle } from "./strategy-pattern-lab";
...
async function main() {
  ...
  const result =
    job === "strategy-shadow-execution"
      ? await runStrategyShadowExecution(db)
      : job === "strategy-signal-producer"
        ? await runStrategySignalProducer(db)
