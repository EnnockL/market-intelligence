...
export const BASELINE_FORECAST_VERSION="historical-cohort-baseline-v2";
export const BASELINE_FEATURE_VERSION="typed-market-features-v2";
export const BASELINE_COHORT_POLICY={version:"same-asset-typed-buckets-v2",minimumSampleSize:30,minimumDataQuality:70,quantileLower:.1,quantileUpper:.9}as const;
export type BaselineAssetClass = "STOCK" | "CRYPTO";
export interface BaselineFeature{assetClass?:BaselineAssetClass;priceMomentum5m:number|null;volumeMultiple5m:number|null;liquidityUsd:number|null;marketCapUsd:number|null;dataQuality:number;}
...
export function featureBucket(feature:BaselineFeature){return{assetClass:feature.assetClass??"CRYPTO",momentum:bucket(feature.priceMomentum5m,[-20,-5,5,20]),volume:bucket(feature.volumeMultiple5m,[.5,1,2,5]),liquidity:feature.assetClass==="STOCK"?"NOT_APPLICABLE":logBucket(feature.liquidityUsd),marketCap:feature.assetClass==="STOCK"?"NOT_APPLICABLE":logBucket(feature.marketCapUsd)};}
export function buildHistoricalBaseline(target:BaselineFeature,examples:HistoricalExample[],horizon:ForecastHorizon,informationCutoffAt="9999-12-31T23:59:59Z"):BaselineResult{
 ...
 const eligible=examples.filter(example=>example.availableAt<=informationCutoffAt&&example.outcomeAt<=informationCutoffAt&&example.anchorAt<example.outcomeAt&&!requiredMissing(example.features)&&example.features.dataQuality>=BASELINE_COHORT_POLICY.minimumDataQuality&&sameBucket(targetBucket,featureBucket(example.features))).sort((a,b)=>a.anchorAt.localeCompare(b.anchorAt)||a.sourceId.localeCompare(b.sourceId));
...
function requiredMissing(x:BaselineFeature){return x.priceMomentum5m===null||x.volumeMultiple5m===null||(x.assetClass!=="STOCK"&&x.liquidityUsd===null);}
...
function sameBucket(a:ReturnType<typeof featureBucket>,b:ReturnType<typeof featureBucket>){return a.assetClass===b.assetClass&&a.momentum===b.momentum&&a.volume===b.volume&&a.liquidity===b.liquidity&&a.marketCap===b.marketCap;}
