...
export type ReplayItemKind = "EVENT" | "CANDIDATE_REVISION" | "QUALIFICATION" | "CATALYST_REVISION" | "FORECAST" | "KNOWLEDGE_RULE" | "KNOWLEDGE_RETRIEVAL";
...
const priority:Record<ReplayItemKind,number>={EVENT:0,KNOWLEDGE_RULE:1,KNOWLEDGE_RETRIEVAL:2,CANDIDATE_REVISION:3,QUALIFICATION:4,CATALYST_REVISION:5,FORECAST:6};
...
export function advanceReplay(items:ReplaySourceItem[],previous:ReplayState|null,until:string,limit=500){
  ...
  const state:ReplayState=previous?{...previous,counts:{...previous.counts,CATALYST_REVISION:previous.counts.CATALYST_REVISION??0,FORECAST:previous.counts.FORECAST??0,KNOWLEDGE_RULE:previous.counts.KNOWLEDGE_RULE??0,KNOWLEDGE_RETRIEVAL:previous.counts.KNOWLEDGE_RETRIEVAL??0},latestCandidateState:{...previous.latestCandidateState}}:{replayTime:until,processed:0,counts:{EVENT:0,CANDIDATE_REVISION:0,QUALIFICATION:0,CATALYST_REVISION:0,FORECAST:0,KNOWLEDGE_RULE:0,KNOWLEDGE_RETRIEVAL:0},latestCandidateState:{},unknownCount:0,cursor:null};
