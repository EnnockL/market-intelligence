...
export type ReplayItemKind = "EVENT" | "CANDIDATE_REVISION" | "QUALIFICATION" | "CATALYST_REVISION" | "FORECAST";
...
const priority:Record<ReplayItemKind,number>={EVENT:0,CANDIDATE_REVISION:1,QUALIFICATION:2,CATALYST_REVISION:3,FORECAST:4};
...
export function advanceReplay(items:ReplaySourceItem[],previous:ReplayState|null,until:string,limit=500){
  ...
  const state:ReplayState=previous?{...previous,counts:{...previous.counts,CATALYST_REVISION:previous.counts.CATALYST_REVISION??0,FORECAST:previous.counts.FORECAST??0},latestCandidateState:{...previous.latestCandidateState}}:{replayTime:until,processed:0,counts:{EVENT:0,CANDIDATE_REVISION:0,QUALIFICATION:0,CATALYST_REVISION:0,FORECAST:0},latestCandidateState:{},unknownCount:0,cursor:null};
