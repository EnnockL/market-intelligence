...
export interface IndependenceEvidence { status: "confirmed" | "related" | "unknown"; evidenceIds: string[]; relationshipTypes: string[]; dataQuality: number; relationshipCoverage?:number; confirmedIndependentCount?:number; clusterAdjustedCount?:number|null; modelVersion?:string; cutoff?:string; }
...
export function evaluateFastFlow(cluster: VerifiedBuy[], independence: IndependenceEvidence, safety: FastSafetyEvidence): FastFlowEvaluation {
  ...
  if(independence.status==="related"&&independence.relationshipTypes.some(type=>["bot_cluster","mev_cluster"].includes(type))){blockers.push(...independence.relationshipTypes.map(type=>`wallet_relationship:${type}`));state="rejected";}
  if(independence.status==="related"&&(independence.clusterAdjustedCount===null||independence.clusterAdjustedCount===undefined||independence.clusterAdjustedCount<FAST_FLOW_POLICY.minimumVerifiedWallets)){blockers.push("independent_wallet_count_below_minimum");state="rejected";}
  ...
  return {state,blockers,walletIds,eventIds,evidenceIds,detectedAt:cluster[0].occurredAt,informationCutoffAt:cluster.map(item=>item.availableAt).sort().at(-1)!,opportunityScore:state==="fast_opportunity"?Math.min(95,70+walletIds.length*5):0,riskScore,dataQuality,
    safetyResult:{policyVersion:FAST_FLOW_POLICY.version,independence:independence.status,relationshipCoverage:independence.relationshipCoverage??null,confirmedIndependentCount:independence.confirmedIndependentCount??null,clusterAdjustedCount:independence.clusterAdjustedCount??null,independenceModel:independence.modelVersion??null,independenceCutoff:independence.cutoff??null,liquidityUsd:safety.liquidityUsd,riskStatus:safety.riskStatus,authorityCoverage:safety.authorityCoverage,blockers}};
}
