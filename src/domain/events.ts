...
const eventEnvelopeSchema = z.object({
  ...
  occurredAt: z.string().datetime(), observedAt: z.string().datetime(), availableAt: z.string().datetime(),
  ...
  if (hashPayload(event.payload) !== event.payloadHash) context.addIssue({ code: "custom", message: "payloadHash does not match payload", path: ["payloadHash"] });
});
...
export function parseEventEnvelope(value: unknown): EventEnvelope { return eventEnvelopeSchema.parse(value); }
