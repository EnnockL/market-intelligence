...
suite("Supabase qualification diagnostics",()=>{
  ...
  it("rebuilds candidate evaluations idempotently",async()=>{const service=new QualificationDiagnosticsService(db),first=await service.run("2026-08-17T23:59:59.000Z"),second=await service.run("2026-08-17T23:59:59.000Z");expect(first.revisions).toBeGreaterThanOrEqual(0);expect(second.created).toBe(0)},20_000);
