...
export function submissionFixture(mode: "SHADOW" | "DEMO" = "SHADOW"): SubmissionEvidence {
  ...
  return {
    ...
    account: { id: "account-1", revision: 1, account_key: mode === "SHADOW" ? "shadow-primary" : `${provider}-primary`, provider, provider_environment: mode, status: "ACTIVE" },
    ...
    safety: { id: "safety-1", intent_id: "intent-1", decision: "PASSED", policy_version: "execution-safety-policy-v1", information_cutoff_at: cutoff, available_at: cutoff, context: { instrumentType: "SPOT", leverage: 1, dataAgeMs: 500, criticalSafety: "PASS", liquidityStatus: "PASS", riskStatus: "PASS" } },
    health: { status: "HEALTHY", credentialsValid: true, tradePermission: true, withdrawPermission: false },
