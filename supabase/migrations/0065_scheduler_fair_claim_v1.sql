-- Rotate due jobs by least-recently-started time so a small cron batch cannot starve jobs.
-- Priority remains a deterministic tie-breaker and existing lease/retry semantics stay intact.
...
begin
  select*into j from scheduled_jobs
  where enabled and status<>'PAUSED' and next_run_at<=p_now
    and(locked_at is null or locked_at+(lease_seconds||' seconds')::interval<p_now)
  order by last_started_at asc nulls first,next_run_at,priority,job_key
