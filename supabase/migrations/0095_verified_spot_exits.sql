-- Existing queued v1 approvals must be re-evaluated under the inventory-aware policy.
create or replace function public.authorize_execution_submission_v2(
  p_order_id uuid,
  p_provider text,
  p_mode text,
...
begin
  ...
  if o.provider_order_id is not null then reason := 'FINAL_PROVIDER_ORDER_ALREADY_EXISTS';
    ...
    select 1 from public.execution_safety_evaluations s where s.id = p_safety_evaluation_id
      ...
      and s.policy_version = 'execution-safety-policy-v2' and s.available_at <= current_at
    ...
    or p_check->>'policyVersion' is distinct from 'execution-safety-policy-v2'
