...
export async function checkExecutionFinalGuard(db) {
  ...
  async function fixture(mode = "SHADOW", options = {}) {
    ...
    const ledgerPayload = { version: ledgerVersion, accountId: account.id, status: "KNOWN", unknownReasons: [],
    ...
    await db.query(`insert into public.risk_ledger_snapshots(id,snapshot_key,account_id,ledger_version,status,cash_sek,open_positions,
      ...
      available_cash_sek,economic_cutoff_at,ledger_payload,evidence_refs,information_cutoff_at,available_at,result_hash)
      values($1,$2,$3,$4,'KNOWN',1000,0,null,null,0,0,100,0,0,900,$5,$6::jsonb,'[]',$7,$7,'fixture')`,
      [ids.risk, snapshotKey, account.id, ledgerVersion, economicCutoff, JSON.stringify(ledgerPayload), cutoff]);
    ...
    await db.query(`insert into public.execution_safety_evaluations(id,evaluation_key,intent_id,policy_version,decision,requirements,context,limits,result_hash,information_cutoff_at,available_at)
      values($1,$2,$3,'execution-safety-policy-v1','PASSED','[{"status":"PASS"}]','{}','{}','fixture',$4,$4)`, [ids.safety, `guard-${ids.safety}`, ids.intent, cutoff]);
    ...
    const check = { decision: "PASSED", guardVersion: "execution-submission-guard-v1", policyVersion: "execution-safety-policy-v1", requirements: [{ code: "FIXTURE", status: "PASS" }], checkedAt: new Date().toISOString(), dataAgeMs: 1000 };
